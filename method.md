...
**v3.0.0** — Last updated: 2026-06-10 11:51  *(Pipekit 3.0 final — native-on-Workflow is the default executor; VBW is an optional backend. Consolidates the rc cycle: rc1 (backend dispatch correction — `/work` plans **inline** in every backend, the `vbw` backend dispatches **only** `vbw-dev`, `vbw-lead` appears only in direct VBW use), rc2 (`/pk-bug` branch-early; backend-agnostic Linear review layer v5.2), rc3 (Ownership-model `vbw-lead` scrub + POC-57 verdict). Final adds no behavior change — it completes the same scrub in `GUIDE.md` + `agents/plan-reviewer.md`. Justified by POC-48 round-two and reinforced by the POC-57 verdict, see `experiments/`. Carries the v2.8.0 + v2.7.x substrate)*
