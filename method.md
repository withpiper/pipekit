...
**v4.0.0-rc3** — Last updated: 2026-06-20 06:56  *(**Linear MCP tool-name migration.** All 19 interactive Linear skills now call the `@tacticlaunch/mcp-linear` camelCase tools (`linear_searchIssues`, `linear_getIssueById`, `linear_createIssue`/`linear_updateIssue`, `linear_createComment`, `linear_getProjects`, `linear_getWorkflowStates`, `linear_createIssueRelation`, `linear_createTeamLabel`) — the server both consuming projects register as `linear-server`. The prior snake_case house names (`list_issues`/`get_issue`/`save_issue`/`save_comment`) matched **no** installed server and silently broke every interactive Linear call; only `bin/pk`'s direct Linear REST path (the high-frequency `pk ship`/`pk done` transitions) was unaffected, which is why the break went unnoticed until `/linear-hygiene` exercised MCP live on SiteLine (2026-06-19). The official `mcp.linear.app` remote uses snake_case and lacks the initiative/label/relation tools the skills rely on — pipekit standardizes on tacticlaunch (documented in `sop/Linear_SOP.md`). Also promotes the previously-Unreleased `/linear-hygiene` placement janitor + `/pk-exit` hygiene check into a tagged release. **Carries** rc2's de-stale+debrand and rc1's executor removal; the VBW planning layer is untouched.)*
