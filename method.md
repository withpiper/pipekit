...
**v3.0.0-rc3** — Last updated: 2026-06-09 22:37  *(Pipekit 3.0 — native-on-Workflow is the default executor; VBW is an optional backend. rc3 scrubs the last stale `/work`-spawns-`vbw-lead` references from the Ownership model (Rule 5, the VBW-steering-surface list, the `/review-plan` row) that rc1 named as a known follow-up — bringing them in line with the already-correct Stage 2 text: `/work` plans **inline** in every backend, the `vbw` backend dispatches **only** `vbw-dev`, and `vbw-lead` appears only in direct VBW use. No behavior change. Carries rc1/rc2 (backend dispatch correction, `/pk-bug` branch-early, backend-agnostic Linear review layer); justified by POC-48 round-two and reinforced by the POC-57 verdict, see `experiments/`. Carries the v2.8.0 + v2.7.x substrate)*
