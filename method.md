...
| `pk ready [<ID>] [--force]` | Flip a Draft PR to Ready (v2.6.0+). Fires `ready_for_review` GH event → outside reviewers (Semgrep + claude-review templates) run. No Linear state change. Refuses on post-verify source/migration drift (v4.27.0+); `--force` flips anyway and logs a Linear comment. |
