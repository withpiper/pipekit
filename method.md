...
| 6a | 3 | **Security Gate** *(v4.4.0; projects with a categories file)* | `/security-gate [<ID>]` | Classifies the feature diff into sensitive categories (auth, payments, user-input, external-APIs, file-storage, PII); on a match, runs the category checklist against the diff → PASS/FAIL report + Linear comment. **Advisory** — does not block. None matched → instant PASS. | Security-sensitive change reviewed before it reaches UAT (the Building → UAT seam) |
