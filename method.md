...
- **`/work <ID>`** does plan + execute in one skill, gated by a **verdict** (`proceed` / `revise: <feedback>` / `abort`) before any code is written. Tier (Quick / Standard / Heavy) is human-confirmed before the verdict step. `/work` **plans inline** (in your current Claude session, with parallel `Agent` grounding), then executes on the **native-on-Workflow** backend — the sole executor as of v4.0.0:
  - `/work` writes a task DAG to `.pk-work/<ID>-PLAN.md`, then hands it as data to the saved **`pk-execute` workflow** (`workflows/pk-execute.js`, synced into every consumer's `.claude/workflows/`). The script holds the loop: one agent per task carrying only its task slice and the issue's intent line, tests authored before the verify runs, a commit only on pass, the expected HEAD sha threaded task to task so an agent that finds a different HEAD refuses to edit, and — when the project has `worktree.baseRef: head` — disjoint-file siblings in parallel worktrees followed by an integration step that cherry-picks the wave and re-runs each verify. The session writes `.pk-work/<ID>-SUMMARY.md` from the structured results. Trivial plans run inline. Scope is the executor contract only — no UAT/known-issue/sprint state.
  - There is no backend selection; a leftover `Backend:` row in `method.config.md` is ignored. `/work --resume <ID>` re-launches the workflow with only the tasks whose commits are not yet on the branch.
...
| `/repo-security-review` | Periodic **whole-repo** security audit — parallel area agents → adversarial verification → evidence-classified score report. Portable framework; project audit areas in `resources/repo-security-areas.md`. No-op without an areas file. Renamed from `/security-review` in v4.31.0 (collided with Claude Code's built-in). |
