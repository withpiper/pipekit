...
**v3.1.0** — Last updated: 2026-06-10 12:16  *(Distribution-layer hardening: `bin/pk` gains its first test coverage (smoke suite + CI gate) and a global `--help` guard (a consumer's `pk ship --help` actually shipped); the sync gains the committed `pipekit/.local-skills` manifest separating local-by-design skills from removed upstream ones; `pk doctor` warns when the synced Pipekit lags the latest release; and the hand-curated visual roadmap (`NEXT.md`/`ROADMAP.md`, human-owned narrative) is re-legitimized as a first-class optional artifact — the machine-readable v1 mirror stays retired, `pk next` stays the truth. Carries Pipekit 3.0: native-on-Workflow default executor, VBW optional, justified by POC-48 round-two + the POC-57 verdict, see `experiments/`)*
