...
| Phase surface | Linear Initiatives (`i{N}.`) → Projects (`I{N}.P{N}.`) → Issues | `pk next`, `pk status`, `/phase-plan`, `/roadmap-create` (live in Linear; no committed file — see `method.config.md § Phase Surface`) |
...
- `/roadmap-create` extracts requirements from strategy docs and authors the **Linear-native phase surface** — Initiatives (`i{N}.` phases) → Projects (`I{N}.P{N}.` sub-phases) → Issues
