...
- **Rate-limit discipline — Linear's quota is a SHARED, per-User resource.** Linear meters requests **per User**, not per key or per process: its docs state requests by the same user share quota across different API keys. An API key gets **2,500 req/hr**, an OAuth app 5,000, on a **leaky bucket** refilling at `LIMIT/PERIOD` (~42 req/min on a key). Complexity is metered separately (object 1 pt, property 0.1, a connection multiplies children by the pagination arg, 10,000-pt single-query cap), so a large page is one request but not a cheap one. Consequences for this skill, which reads the *whole board*:
  - **Never fan out parallel subagents at `linear-server`.** One subagent, serial calls. Anchor: SiteLine 2026-08-31 — three concurrent sweeps paging 250 exhausted the quota, blocked the board ~1h, and starved four active worktree workers. **Every Claude session spawns its own `mcp-linear` server 1:1**, so on a machine running worktree workers the quota is already contended before this skill makes its first call.
  ...
  - **Prefer running this skill when no `/work` sessions are live.** Check with `pgrep -f 'bin/mcp-linear' | wc -l`.
...
- **Issue the updates serially, not in parallel.** These are writes against a rate-limited API (see Rate-limit discipline above); a fan-out here is the single easiest way to blow the quota mid-apply and leave the board half-updated.
