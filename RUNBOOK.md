...
| **5a** | **Flip Draft → Ready** | **`pk ready [<ID>]`** | **`./bin/pk ready [<ID>]`** | **worktree or parent** | **fires `ready_for_review` GH event → outside reviewers run (v2.6.0+). Refuses if source/a migration changed since `/verify` (v4.27.0+) — re-verify or `--force`** |
