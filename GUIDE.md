...
- **Stale verify evidence now blocks the flip (v4.27.0+).** `pk ship` proved a verify sentinel matched HEAD *at ship time*. Commits you push afterwards — review fixes, UAT fixes — move HEAD past it, and until now nothing re-checked, so a PR could merge carrying a PASS artifact that described code no longer in the diff. `pk ready` compares the newest verify evidence for the issue against current HEAD and classifies what changed since: evidence/prose only is benign and passes; **source or a migration refuses the flip**. Migrations get their own message, because a migration edited after the verify that exercised it also breaks the frozen-file invariant (the freeze starts at first application to *any* database, local included) — and the fix for that is a new migration, not an edit. Re-run `/verify`, or `pk ready --force` to flip anyway, which logs a Linear audit comment the same way `pk ship --force` does. The check is computed *before* the already-Ready early return, because the case that motivated it (SiteLine PIPER-490) was a PR that had already been flipped when the migration changed under it. Evidence lookup falls back past `verify-complete.md` to `evidence.txt` / `reality-check.md` — PIPER-490 had no `verify-complete.md` at all, and keying only on the sentinel would have left this gate blind to its own anchor case
