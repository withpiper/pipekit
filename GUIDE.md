...
**v4.32.0** — Last updated: 2026-08-26  *(**v4.32.0 — `pk done` / `pk ready` find follow-up branches, and a second ship reads as one.** Both globbed `feature/<ID>-*` alone, so the fix branch the loop legitimately produces after UAT was invisible: `pk done` errored "no local feature branch found" (which reads as "unknown issue"), the follow-up PR never reached the Linear ship record, and the branch was never cleaned up — all five multi-PR issues across 185 ID-prefixed merged PRs on SiteLine had this shape. Both commands now share one resolver (`feature|fix|hotfix`, newest commit first, remote fallback included — `cmd_done` had none), and a second `pk done` posts **Shipped (follow-up)** rather than erroring. Carries v4.31.3 — skill overrides now cover the whole skill directory, not just `*.md`. Carries v4.31.2 — `/pk-bug` Phase 8's Urgent sign-off gate was unreachable. Carries v4.31.1 — promote's issue-bundler stopped matching prose.)*
...
- **Stale verify evidence now blocks the flip (v4.27.0+).** `pk ship` proved a verify sentinel matched HEAD *at ship time*. Commits you push afterwards — review fixes, UAT fixes — move HEAD past it, and until now nothing re-checked, so a PR could merge carrying a PASS artifact that described code no longer in the diff. `pk ready` compares the newest verify evidence for the issue against current HEAD and classifies what changed since: evidence/prose only is benign and passes; **source or a migration refuses the flip**. **So does evidence pointing at a commit that no longer exists** (rebase, squash, force-push): drift is then uncomputable, and uncomputable is not clean. That case fails closed on purpose — `pk ship`'s own gate demands an *exact* sha match, so a rebased branch cannot ship at all, and letting it flip to Ready would make this gate looser than the one it reinforces. A rebase immediately before requesting review would otherwise defeat the feature entirely. Migrations get their own message, because a migration edited after the verify that exercised it also breaks the frozen-file invariant (the freeze starts at first application to *any* database, local included) — and the fix for that is a new migration, not an edit. Re-run `/verify`, or `pk ready --force` to flip anyway, which logs a Linear audit comment the same way `pk ship --force` does. The check is computed *before* the already-Ready early return, because the case that motivated it (SiteLine PIPER-490) was a PR that had already been flipped when the migration changed under it. Evidence lookup falls back past `verify-complete.md` to `evidence.txt` / `reality-check.md` — PIPER-490 had no `verify-complete.md` at all, and keying only on the sentinel would have left this gate blind to its own anchor case
- **Follow-up branches resolve, not just `feature/` (v4.32.0+).** `pk ready <ID>` and `pk done <ID>` used to glob `feature/<ID>-*` alone, so the second branch the loop legitimately produces — feature merges, UAT surfaces something small, the fix lands on `fix/<ID>-*` — was invisible to both. Both now search `feature/`, `fix/` and `hotfix/`, newest commit first (a follow-up is by definition newer than the branch it follows), and share one resolver so they cannot disagree about where a branch lives. The not-found message names every glob it tried, because the old text named only `feature/<ID>-*` and read as "I don't know this issue" rather than "I looked in the wrong place". Anchor: across 185 ID-prefixed merged PRs on SiteLine, *all five* multi-PR issues had this shape — POC-448 lost two PRs off its ship record, POC-450 got no record at all, and POC-426 and PIPER-782 were patched by hand five weeks apart without the cause being filed (PIPER-793)
...
**Accept:** Merge the PR (rebase or merge-commit; squash is disabled repo-wide). Then exit the worktree and run `pk done <ID>` from the parent repo — this verifies the merge, cleans up the worktree + branch, posts commits + diffstat to Linear, transitions Linear `UAT → In <FirstEnv>` (e.g. `In Dev`, or → `Done` for 1-tier projects), and auto-pulls the integration branch (v2.6.0+). Or pass `--merge` and let `pk done` run `gh pr merge` for you. v4.32.0+ resolves `fix/`/`hotfix/` branches as well as `feature/` (see `pk ready` above), falls back to `origin` when the local branch is already gone — announcing that there is nothing local to clean up rather than quietly posting a thinner record — and heads the Linear comment **Shipped (follow-up)** when the issue already carries a ship record, so an issue that shipped twice reads as one issue with two PRs instead of two first ships. v2.7.0+ also resets the parent branch after worktree teardown, prints a stack advisory (a local dev server / Supabase keeps running), and — for script-deploy projects with a `Deploy command` set in `method.config.md` — reminds you to deploy (the merge doesn't auto-ship the app).
