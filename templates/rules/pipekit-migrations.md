...
**Historical incident:** Piper, 2026-05-27. An author applied `20260527160000_revoke_default_public_grants.sql` via `mcp.apply_migration` on `piper-dev`. The MCP server stamped the remote history with `20260527164211` (wall-clock at apply). The disk file went into a PR with timestamp `20260527160000`. CI's `db push` on merge refused with the history-drift error. ~5h of downstream deploy lockup followed while every other in-flight migration PR was blocked. Reconciliation required `supabase migration repair` against `piper-dev`. The pattern was previously informal team knowledge; this rule makes it explicit.
