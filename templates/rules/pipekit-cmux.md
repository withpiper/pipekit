...
Better still, wire the worker's `Stop`/`Notification` hooks to `cmux notify` (see the Pipekit-specific patterns above) so turn-end signals the sidebar without any polling. Anchor: 2026-06-03, watching a `/pk-express` worker — a keyword watcher false-fired three times on the worker explaining its own gates before turn-end detection caught the real stops.
