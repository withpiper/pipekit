...
1. ⬜ **Merge-driven Linear transition (the real fix).** A GitHub Action on `pull_request: closed` + `merged == true` against the integration branch that extracts the issue ID from the branch name **and** the PR title/body/commits, and transitions each to **In `<FirstEnv>`**. Removes the dependency on a human running `pk done`. Mirror it for downstream-env / final merges. (Piper already runs `supabase-production.yml` on beta-merge — the same event hook can carry the Linear transition.) Belongs in `templates/ci/`.
