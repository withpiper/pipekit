...
1. 🟡 **Merge-driven Linear transition (the real fix). — DRAFTED (Unreleased, branch `feat/linear-transition-ci`).** Shipped as `templates/ci/linear-transition.yml`: a GitHub Action on `pull_request: closed` + `merged == true` against the integration branch that extracts every `<PREFIX>-NNN` from the branch name + PR title + body and transitions each to the configured `TARGET_STATE` (`In <FirstEnv>` multi-tier, `Done` single-tier) via the Linear GraphQL API — same auth + `workflowStates`/`issueUpdate` shapes as `bin/pk`. Idempotent (skips a WIT already at target — harmless alongside `pk done`) and forward-only (a `PRE_MERGE_STATES` allowlist prevents pulling a promoted/Done WIT backward and prevents leap-frogging an Approved WIT that bypassed UAT). README documents per-project setup (team/prefix/target-state `env` block + `LINEAR_API_KEY` secret). **Still open before this can close:** (a) commit messages are NOT parsed for IDs yet — only branch + title + body (parsing commits would catch a squash-merged bundle whose extra IDs live only in commit subjects); (b) downstream-hop coverage is per-file/manual (one workflow watches one hop); (c) not yet validated against a live merge. Piper already runs `supabase-production.yml` on beta-merge — the same event hook could carry the transition if a project prefers one workflow.
