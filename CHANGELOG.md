...
Canonical source is `templates/rules/`; consumers inherit R1/R2 on their next `sync-method.sh`. No `bin/pk` logic change (`PK_VERSION` bump only) — smoke unchanged at **121**. Rider: cleared pre-existing `VBW`→`/work` drift between this repo's gitignored `.claude/rules/pipekit-discipline.md` copy and its `templates/rules/` source.

**Scoped + deferred:** R5 — a lean `pk issue show <ID> --no-comments --fields …` REST read wrapper (the only piece touching the CI-gated `bin/pk`) — gives skills a low-token structured read and removes the temptation to reach for the fat MCP call. Spec in `resources/mcp-payload-hygiene-followup.md`; the documented fast-follow, same pattern as the v4.4.0/v4.3.0 advisory-gate → v4.17.0 hard-gate deferral.
