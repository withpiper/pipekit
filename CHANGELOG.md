...
| `sop/Lane_Map_SOP.md` | SOP — `/lane-map` conventions: frontier rule, artifact constraints, the scaffold-once split |
...
>
> **`/verify` cites only this run's evidence.** A session resumed after a machine pause closed with "fresh artifacts pass the checker, 1,940 checks" — a claim from the pre-pause `/work` step whose artifacts had been wiped with the scratchpad; the run's `evidence.txt` had no such leg. Step 9 now states the rule the evidence layer always implied: a check cited anywhere must be a `==> $` block in this run's `evidence.txt` at this sha, or it is reported as not run. Failure-model rows for a mid-run resume and a dead adversarial subagent.
>
> **`/pr-security-review` reads wrapped `Keywords:` lines.** The Auth-helper extractor took the first `Keywords:` line only; SiteLine's categories file wraps that line, so the second half of its signals (RS256, CREATE POLICY, SECURITY DEFINER, `auth.uid()`) never reached the grep. Continuation lines are joined now.
>
> **Model Policy, with data.** The PIPER-412 A/B — `/work` v4.32.0 (which chose `Mode: inline` for a four-task plan and so executed on the session's frontier model) against v4.34.0 on `sonnet`/`medium` workflow agents, three blind opus judges, protocol and results in `resources/ab-work-PIPER-412-2026-09-02.md`. At the `/work` cut: equal AC coverage, the candidate preferred 72/100 for tests that execute the builders and a green first gate (the incumbent's first gate run was red on a lazy-load pin its Step 6.5 full gate would have caught). The ship went to the incumbent branch only because its `/verify` had iterated twice on opus. Execution stays `sonnet`/`medium`; the reviewer tier decides. Open, measured, not fixed: v4.34.0's main-session planning + integration cost as much as the incumbent's whole inline execution (98k vs 94k output tokens) before any agent ran. Parallel waves were off in both arms and remain untested.

## v4.35.0 — 2026-09-02

> **The commit-format check now blocks, instead of nudging after the fact.** `validate-commit.sh` was a PostToolUse hook that always exited 0 — and a PostToolUse hook *cannot* block or undo, the tool already ran. Its header claimed it paired with a commit-format rule in the consumer's rules and CLAUDE.md; no such rule existed. The format lived only in the demand-loaded Git SOP and three skills, and `pipekit-discipline.md` § Commit discipline never stated the shape. Anchor: piper PR #771, 2026-09-02 — the nudge fired on `docs: runbook records …`, the session pushed with a raw `git push` (no Linear issue, so no `pk ship`), the PR merged, and the unscoped subject is permanent on `dev`, where squash is off. The nudge was the only guard and it did not hold.
>
> **Four changes, one contract: the AI cannot commit off-format, and nothing off-format ships.** (1) The rule is in the auto-loaded `pipekit-discipline.md`, and the hook header stops claiming a rule that wasn't there. (2) The PostToolUse text is an instruction — the commit is still local, amend now, before any push — not a report. (3) The same script is registered a second time as **PreToolUse** and denies an off-format `git commit` before it runs, via the documented `permissionDecision: "deny"` with the reason carried to the model; the commit never exists. The sync registers per event, because the old any-event guard would have reported "already registered" and left PreToolUse unwired on every existing consumer. There is deliberately no `if: "Bash(git commit *)"` filter: permission patterns match per `&&`-separated subcommand and the hooks docs do not say whether a hook `if` fires on any-match or all-match, so `cd dir && git commit …` — the usual shape in a session whose cwd resets between calls — might never reach a filtered hook; the script's own first-line grep is the filter, and the false-positive corpus (grep patterns, echoed examples, heredoc prose in a PR comment) is covered under PreToolUse too, since a false positive now blocks a tool call rather than annoying. (4) **`pk ship` reads the real subjects** in `<base>..HEAD` (merge commits skipped) and refuses to push an off-format one — zero parse surface, and it catches a commit the hook never saw, including a human's. `--force` waives with a Linear audit comment, as for the verify gate; `PK_COMMITFMT_BYPASS=1` for emergencies. A git-native `commit-msg` hook installed via `core.hooksPath` was considered and rejected: per-clone config the sync would have to re-assert, and it redirects *every* git hook, silently disabling piper's existing pre-push.
>
> **`pk status` and `pk next` no longer render an empty board when the key is unset.** Both swallowed per-state read errors on purpose (one flaky call must not blank the board), which meant a missing `LINEAR_API_KEY` printed every column `(none)` and `No Approved issues.` — indistinguishable from a genuinely idle board. Found while picking A/B candidates: both consumers read as idle while carrying 290 open issues between them. Both commands now refuse with the same error `pk issue show` already gave. `sop/Hooks_SOP.md` is rewritten — it said no hook ships and none are synced, both false since the hook first shipped.
>
> Smoke 265/265 (was 244), each new guard mutation-checked: breaking the deny, the per-event registration, the ship gate, or the key check reddens exactly its own tests. drift-check 33 errors / 52 warnings (was 33 / 53 — the rewritten Hooks SOP is no longer flagged stale).

## v4.34.0 — 2026-09-02

> **`/work` executes on a saved workflow instead of a prose description of one.** Step 5 told the model to "drive execution with the Workflow tool" whose script "reads the task DAG from `.pk-work/<ID>-PLAN.md`" — and workflow scripts have no filesystem access, so that step could never run as written; every real `/work` run has been the sequential fallback. The loop now lives in **`workflows/pk-execute.js`**, a Claude Code saved workflow that `sync-method.sh` ships to each consumer's `.claude/workflows/` (a tracked path, so it is present in every `pk` worktree — the `.mcp.json` argument) and `dogfood-sync.sh` mirrors for pipekit's own sessions. The session parses the PLAN and hands the DAG in as `args`; the script orders tasks by `deps`, runs one agent per task with only its task slice and the issue's intent line, has each agent author its tests before running its verify and commit only on pass, and threads the **expected HEAD sha** from task to task so an agent that finds a different HEAD returns `wrong-base` and touches nothing. It stops at the first task that is not `done` and returns structured results; the session writes `.pk-work/<ID>-SUMMARY.md` from them, so bookkeeping is code and the model keeps the judgment. Three new smoke tests: the meta block is the first statement, the script parses (wrapped in an async IIFE — `node --check` rejects a bare top-level `return`), and a dry-run sync ships a fixture workflow.
>
> **Parallel tasks are real now, and gated on the one setting that makes them safe.** The Agent tool's `isolation: "worktree"` was a silent no-op on 2026-05-13 (four "parallel" agents shared one checkout on rs-vault and discarded each other's work) and `/work` said so. Re-verified 2026-09-02 on Claude Code 2.1.258: it creates a real worktree under `.claude/worktrees/` on a `worktree-<name>` branch. But the harness bases that worktree on **origin's default branch** unless `worktree.baseRef` is `head` — which, inside a worktree, resolves to that worktree's HEAD — so a task agent would otherwise build on `main` rather than on the previous task's commit. `/work` sets `parallel: true` only when the setting reads `head` and `.claude/worktrees/` is gitignored, and says which check failed when it is not; `pk-execute` then runs disjoint-`files` siblings at one dependency level in parallel worktrees and follows the wave with an **integration agent** that cherry-picks in task order and re-runs every task's verify on the integrated tree. Otherwise every task runs sequentially in the issue's worktree — the production-validated path since v3.0.0-rc1, and the right default on a rate-capped plan. `/06-linear-todo-runner`'s three "no-op" claims are corrected; it keeps its explicit `git worktree add` because it needs sibling paths, `pk`-convention branch names and a base it chooses.
>
> **The skill is de-prescribed for Claude Fable 5.1.** Anthropic's migration guidance is explicit that prompts and skills written for prior models are often too prescriptive and *reduce* output quality, and names the shapes: numeric output caps, delegation-suppressing guardrails, prohibition clusters without provenance, history narratives. Gone: the `≤200` / `≤300` word caps on the grounding prompts; the "sequential by default, parallel is a deliberate optimization, when in doubt serialize" framing (the Max-plan rate cap survives as the reason `maxParallel` is 3, not as a reflex); the v1 comparison table; the rationalization table that restated `pipekit-discipline.md` rows the session already auto-loads. Kept verbatim, because the guide is equally explicit that verification instructions and fragile exact scripts stay: Step 0's worktree bash, the three self-reference greps, the flag-marker contract, the two mechanical rollover skips, and the `pk done` / `pk promote` boundaries with their WIT-451 anchor. Added, because the same guide documents them: an intent line in every task-agent prompt, grounded progress claims, structured returns via `schema`, and a definition of `--resume` (referenced twice before, defined nowhere) — re-launch with only the tasks whose commits are not yet reachable from HEAD, never the runtime's same-session replay cache, since a human fix between runs is the normal case. 602 → 340 lines; after auto-compaction the harness re-injects an invoked skill capped near 5K tokens, so the old file lost half its body mid-run and the new one fits.
>
> **Two stale claims closed on the way.** v4.30.0 said `/work` dropped the `Backend` refusal; only the failure-model rows went, and Step 1 still read the key and refused on any non-`native` value — a leftover `Backend: vbw` row was *not* inert for `/work`, contrary to that release's log. And Step 6.5's risk-fallback filing called `mcp__claude_ai_Linear__save_issue`, the one use of the claude.ai connector's tool names across all 36 skills, which fails in any consumer without that connector; it is `linear_createIssue` on the tacticlaunch server like everything else.
>
> **Model Policy note.** The execution row keeps `sonnet` / `medium` by the silent-miss rubric, but may now be left blank to inherit the session model: on current frontier models `low` / `medium` effort on the session model can cost less per *completed* task than a smaller model at `medium`. The template says to measure before flipping. Validation still owed: an A/B of one Standard issue per consumer on the de-prescribed skill — removal is a hypothesis. pk-smoke: 244/244 (was 241); drift-check 33 errors / 53 warnings (was 35 / 51 — errors down two; the two added warnings are commit-count staleness on templates this release did not touch, since that metric counts commits since a file last changed).

## v4.33.0 — 2026-09-02

> **RECONCILE gains the class the loop was missing: "AC wrong."** The completion-claims loop classified adversarial findings as *AC misread → valid + actionable → trade-off → noise*, which left no legal home for the finding that the artifact is right and the **spec** is stale. SiteLine PIPER-770 (2026-08-25) shows what happens then: a pre-merge review caught the code contradicting its own spec's confirmation contract, recorded the verdict "honest UI, unamended spec", classified it a trade-off and deferred it — so neither half was amended, the behaviour shipped, priced things wrong for a day, and came back as its own Medium bug. Detection worked, 7 hours before merge. Classification failed. The class now sits second in precedence, and a **closure rule** says an artifact-vs-AC contradiction resolves *only* by changing the artifact or amending the AC — never by shipping with both standing. The doubt-theater flag counts a class-2 amendment as a real correction, since what it hunts is a loop where every finding is absorbed as a trade-off or dismissed as noise. `pipekit-discipline.md`, `sop/Completion_Claims_SOP.md`, and `/verify` restate one list; `/verify` stops re-enumerating it a third time.
>
> **Linear's quota is per-User and shared by every session on the machine.** Each Claude session spawns its own `mcp-linear` server 1:1, all as the same user, so worktree workers, the parent-repo session, and a board sweep draw from one 2,500 req/hr leaky bucket. Anchor: SiteLine 2026-08-31 — three concurrent `/linear-hygiene` sweeps paging 250 exhausted it, blocked the board ~1h, and starved four live workers. The facts and rules — serial calls, ~50 per page, wait once / retry once / then stop and report partial, the error is HTTP 400 `RATELIMITED` not 429, `isBlocked: false` says nothing about remaining quota — are canonical in `sop/Linear_SOP.md` § Rate limits. `/linear-hygiene` issues its apply-phase writes serially and points there; `/06-linear-todo-runner`, whose four workers multiply the draw, warns about it in its prerequisites.
>
> **`/pr-security-review` detects auth code by content, not only by path, and gains an LLM-prompt surface.** A handler that calls a shared auth helper lives anywhere; the path filter (`auth/`, `middleware`, `session`) missed it. The helper names are the *project's*: read from the Auth category's `Keywords:` line in its `Security categories` file, escaped for `grep -E`, with a generic fallback set — the first cut hardcoded SiteLine's helper names into the portable skill, the same shape v4.31.0 removed from `/repo-security-review`. New rubric **L1–L4** covers caller-supplied or DB-sourced text interpolated into a prompt: standing instructions in `system` with an explicit hierarchy, a per-request non-guessable delimiter scrubbed from the untrusted text *before* wrapping, the project's existing neutralizer rather than a fork, and length bounds applied *after* any step that can grow the string.
>
> **`pk done` / `pk ready` resolve every prefix in the naming convention.** v4.32.0 widened the resolver from `feature/` to `feature|fix|hotfix`; the convention also has `refactor|chore|docs`. `PK_BRANCH_PREFIXES` is now the documented override (`pk help`, RUNBOOK), hardened against the two ways a caller-controlled word list goes wrong: a whitespace-only value is treated as unset rather than word-splitting to zero prefixes, and the list is split with globbing off so a value containing `t*` cannot splice cwd filenames in as prefixes. Each prefix, the override, the whitespace case, and the glob case are covered. pk-smoke: 241/241 (was 235).

## v4.32.0 — 2026-08-26

> **`pk done` and `pk ready` only ever looked for `feature/<ID>-*`, so a follow-up fix left an incomplete ship record.** Both located the branch with one hardcoded glob. The daily loop legitimately produces a second branch — the feature PR merges, `pk done` runs, UAT surfaces something small, the fix lands on `fix/<ID>-*` and merges hours later — and that branch was invisible to both commands. `pk done <ID>` failed with *"no local feature branch found matching feature/<ID>-\*"*, which reads as "I can't find this issue" and actually means "I only look for one prefix, and that branch is gone". The follow-up PR never reached the Linear ship record and its local branch was never cleaned up, silently, on both the first run and the retry.
>
> This is the only shape the failure takes, not an edge case. Across 185 ID-prefixed merged PRs in `ethan-piper/SiteLine`, five issues have more than one merged PR and **all five** are `feature/` merging first with a `fix/` branch merging later: POC-448 cites one PR of three, POC-450 has no ship record at all, POC-426 and PIPER-574 each lost one, and PIPER-782 was patched by hand on 2026-08-26 — five weeks after POC-426 was patched by hand the same way, without the cause being filed either time.
>
> **Fix: one shared resolver, `pk_branch_for_issue`.** It searches `feature/`, `fix/` and `hotfix/` for the ID and takes the **most recently committed** match rather than `head -1` on git's listing order — a follow-up fix is by definition newer than the branch it follows, whereas alphabetical order puts `feature` before `fix` and hands back exactly the wrong branch. `cmd_done` also gains the remote fallback `cmd_ready` already had: the two commands disagreeing about how to find a branch was half the bug, and `cmd_done` gave up whenever the local branch was gone even for a correctly-named `feature/` branch. When resolution comes from `origin`, `pk done` now says so — nothing local to clean up, and the Linear comment will carry the PR but not the commit list — rather than quietly posting a thinner record. The not-found message names every glob it tried.
>
> **A second ship now reads as a follow-up.** `pk done` checks whether the issue already carries a pk-authored ship record and heads the comment **`Shipped (follow-up) — <ID>`** when it does, so an issue that shipped twice reads as one issue with two PRs instead of two first ships. The check matches the exact header rather than a substring — `contains("PK-1")` would have let PK-10's record mark PK-1 a follow-up — and fails closed: no API key, an unreachable Linear, or a malformed response all mean "first ship", because mislabelling a genuine first ship is the worse error.
>
> One trap worth recording, since it is the class this repo keeps hitting. The resolver's candidate pipeline was originally `grep -v | sort -rn | head -1`; under the `set -euo pipefail` at `bin/pk:28` a `grep` matching nothing returns 1, so the function exits non-zero on exactly the input that has to fall through to the remote lookup. Measured on bash 3.2.57, that aborts a **bare** call but not one inside `$( )` — and both call sites substitute, so it would not have broken anything today. A latent hazard for the next caller, then, not a live bug; recorded because the *first* version of the test could not tell the difference. Asserting on captured output sees nothing (both forms print nothing), so the check was decorative until it asserted the **exit status** — restoring the fragile pipeline now reddens two tests and previously reddened none. Extracted as `pk_branch_pick_newest`, built from `awk` and `sort`, which exit 0 on empty input however they are called.
>
> Not addressed: backfilling the ship records on POC-448, POC-450 and PIPER-574 — data cleanup, not tooling. `pk branch` still creates only `feature/` branches; this release makes the other prefixes *resolvable*, not *produced*. 22 new smoke tests, each mutation-checked. pk-smoke: 235/235 (was 213).
...
>
> **Fix: subject line, or a `Carries:` manifest trailer, never free body prose.** New shared helper `pk_extract_bundled_issue_ids(range, issue_prefix)` matches an issue ID only in a commit's subject line (the normal case — a commit owns the issue it closes) or in a body line starting exactly `Carries:` (a deliberate, structured, comma-separated manifest — the one legitimate case where a single commit vendors many already-reviewed issues' work at once, e.g. a legacy-tier sync publish). All three call sites in `cmd_promote` now share this one helper instead of three independent `git log --pretty=format:%s%n%b` / `jq '.commits[].messageBody, .commits[].messageHeadline'` greps. Consuming projects whose sync-style commits bundle multiple issues per commit need to adopt the `Carries:` trailer convention going forward — free-prose enumeration ("Carries the delta: X, Y, Z") no longer gets parsed.
>
> `bin/pk` behavior fix only — no skill/doc/template changes.

## v4.31.0 — 2026-08-07

> **The canonical security-audit skill was one project's checklist wearing a portable skill's clothes.** `skills/security-review/SKILL.md` shipped as a portable skill, and its § 2 audit agents hardcoded **SiteLine's** stack: PHP endpoints under `src/app/api/` composing `api-auth.php` / `cors.php` / `rate-limiter.php`, `requireAuth()` on HTML pages, `.htaccess` header checks, and three artifact paths (`Security/SECURITY_ARCHITECTURE.md`, `Security/Security_Score_*.md`, `src/marketing/security.html`). Every project syncing Pipekit inherited that verbatim — auditing primitives it doesn't have and reporting a clean sweep for the ones it does. The provenance is instructive: SiteLine once carried a *local override* of this skill whose generic enhancements were upstreamed in v3.x (finding-stage coverage, grounded reads, the adversarial verification pass); the project-specific checklist rode along in the same lift and nobody separated the two halves. **A skill can be portable in its machinery and parochial in its content, and only the machinery gets reviewed as if it were the contract.**
>
> **Genericized on the `/financial-review` pattern** — the closest existing precedent (periodic cadence, framework-here/substance-in-a-project-file, optional lifecycle). Audit **areas**, their **path globs**, their **primitives** (the repo's actual guard/middleware/policy names to grep for), and their **checks** now live in a project areas file: new key `Repo security areas` → `resources/repo-security-areas.md`, scaffolded from the new `templates/repo-security-areas.template.md`, with a Step 0 gate that stops if it's absent. **One area = one parallel audit agent**, so an area the project adds gets audited and one it omits demonstrably does not. Where an area's checks are thin the skill falls back to four generic baselines (authn/authz, API & input handling, headers/transport/infra, data protection & storage) and **labels that fallback in the report** (`generic-only — no project checks defined`) so an incomplete areas file is visible rather than an invisible pass.
>
> **The three artifact paths are optional keys, and blank means `n/a` — never "done".** `Security architecture doc`, `Public security page`, and `Security scan host` all default blank; the corresponding steps are recorded as `n/a (not configured)` and the skill never writes to a path the project didn't configure or reports a scan it didn't obtain. `Security scan host` is its own key rather than a read of § Environments, because that section carries no `| **Key** |` row and therefore can't legally be cited by a skill under the v4.28.1 rule — the smoke guard added that release is what surfaced the constraint.
>
> **The load-bearing machinery is carried verbatim, deliberately.** High-reasoning-effort / dynamic-workflow framing; the **finding-stage coverage rule** ("maximize coverage; do not self-filter" — without it a capable model investigates deeply and under-reports); **grounded reads** (`file:line`, never infer a vulnerability *or* an all-clear from a filename or from memory); the **adversarial verification pass** (try to *refute* each candidate against real auth/RLS/call-sites; confirmed / refuted / needs-more-info, refuted kept in an appendix rather than deleted); evidence-type classification (`repo_evidence` / `live_external_evidence` / `unverified`); the severity rubric and key principles. That is the part worth having, and none of it was project-specific.
>
> **Renamed `/security-review` → `/repo-security-review`,** because `security-review` collides with **Claude Code's own built-in `/security-review`** (diff-scoped review of pending branch changes) in every consuming project's skill list: two same-named entries doing different jobs. `/security-audit` was the obvious alternative and is taken — SiteLine and Piper both shipped project-local `/security-audit` skills — so the name mirrors its existing sibling instead: `/pr-security-review` and `/repo-security-review` put the scope in the name and sort adjacent in every listing. Division of labor is now explicit in all three skills' triggers and descriptions: **`/security-gate`** (the feature's diff, at the ship seam) · **`/pr-security-review`** (a PR, antagonistic) · **`/repo-security-review`** (the whole repo, periodic).
>
> **One pre-existing defect surfaced by the rename and fixed rather than propagated.** `sop/Production_Readiness_SOP.md` § Overlap credited the *feature-scoped* rate-limit check ("this feature's login route is itself rate-limited against brute force") to the whole-repo periodic audit — but `skills/security-gate/SKILL.md` § Overlap with `/prod-ready` documents that same overlap as `/security-gate`'s, and cites this SOP as its counterpart. The two halves of one cross-reference contradicted each other. Repointed to `/security-gate`.
>
> **Deliberately not renamed:** `/work --deep`'s `subagent_type: "security-review"` (and the prose describing "security-review subagents" in `GUIDE.md`, `RUNBOOK.md`, `sop/Skills_SOP.md`). That string is a **subagent-registry name**, not this skill — `/work` falls back to `general-purpose` when a project hasn't registered one — so renaming it would silently break projects that have. Only slash-command references moved.
>
> Docs, skills, and templates only — no `bin/pk` behavior change (version bump only). Smoke **200**, unchanged.

### Migration (consuming projects)

The rename is a **breaking change for consumers** — `sync-method.sh` copies skills in but never deletes, so the old directory persists until removed by hand.

1. **After syncing v4.31.0**, `.claude/skills/repo-security-review/` appears and `.claude/skills/security-review/` remains. The sync changelog flags the stale one under *"Not in upstream (undeclared)"* with the exact removal command — that warning is the intended signal, not a bug. Delete it: `rm -rf .claude/skills/security-review`.
2. **Scaffold the areas file** on first run: `/repo-security-review` copies `pipekit/templates/repo-security-areas.template.md` to `resources/repo-security-areas.md` and stops for you to fill it in. Until it exists the skill is a no-op — it will not fall back to a generic audit, because a generic audit of an unknown stack is the failure this release exists to remove. Fill in **primitives** first; the checks are worthless without the real names.
3. **Set the optional keys** in `method.config.md` only if the project has those artifacts. A project with no public security page and no deployed host is fully supported and gets `n/a` rows, not failures.
4. **SiteLine and Piper specifically** (both shipped a project-local `/security-audit` plus a redirect-stub override at `.claude/overrides/skills/security-review/SKILL.md`, per SiteLine#824 / piper#599): the stub pinned a name upstream no longer ships, so **retire both halves together** — `rm -rf .claude/skills/security-review .claude/overrides/skills/security-review` and drop the row from `.claude/overrides/MANIFEST.md`. Order doesn't matter, but leaving the override behind does: `apply_override` skips a missing target rather than recreating it, so the stub won't come back — it will just print `SKIP override skills/security-review/SKILL.md (target missing)` on every sync from now until someone deletes it. `/security-audit` stays a local skill and does **not** collide with `/repo-security-review`, so `pipekit/.local-skills` needs no change. Their local audits remain the product-wide, two-tree, strangler-fig-aware ones; this upstream skill is the portable single-repo baseline.
## v4.30.0 — 2026-08-05
...
> **A skill's value can be curation no upstream template can carry, and Pipekit had no sync mode for that.** Fully portable skills get overwritten on every sync, so a skill whose value is project-specific curation — parallel-safe groupings, a collision register, an artifact URL — loses that curation the moment upstream ships an unrelated fix. Fully local skills (`pipekit/.local-skills`) solve that but invert the problem: nothing upstream can ever reach them again, which is exactly how v4.28.0's frontier/head-of-lane rule (wrong for 24 hours — see v4.28.0's own correction) would have shipped wrong forever in every project that had scaffolded a copy before the fix. Anchor: SiteLine's `Pipekit_Handover_LaneMap_2026.08.03_v1.md` — SiteLine built exactly this skill, `/lane-map`, a private web artifact rendering the board's frontier and lanes, and hit the trap directly. **New third sync mode — scaffold-once:** a skill listed in the method repo's own `.scaffold-once-skills` (new manifest, method-repo-side — a consuming project has no manifest of its own to edit, since it can't know in advance which upstream skills get this treatment) is seeded into `.claude/skills/<name>/` once, on the first sync where that directory is absent, and never touched again once present, even as the upstream template keeps changing. `scripts/sync-method.sh` reads the manifest from the freshly-cloned `$TEMP`, not from the consumer. On first scaffold, the skill's name is auto-appended to the project's own `pipekit/.local-skills`, so the pre-existing "possibly upstream-removed" check treats it as project-owned from that point on and the sync changelog reports it under "Project-local." **The pattern requires a split file, or scaffold-once just relocates the propagation problem instead of solving it:** the portable half (conventions, patterns, gotchas) lives in a normally-synced SOP; the scaffolded skill cites that SOP instead of restating it — the hub-and-spoke split already used for `.claude/rules/`, applied to skills for the first time. `/lane-map` is the worked example: `sop/Lane_Map_SOP.md` (new) carries the frontier rule, the board-first-then-map principle, read discipline (per-initiative queries, never `getIssueById` — `pipekit-tooling.md § MCP Result Payloads Are Sticky`), artifact constraints (CSP, no embedded credentials, the `mcp` runtime-capability path, stable URL + favicon), and the snapshot-vs-live tradeoff documented as a deliberate default rather than a TODO; it cross-references `Linear_SOP.md § Board shapes` for what a run-order head is rather than restating it — a direct application of the PIPER-486 anchor this same handover invoked. The scaffolded `skills/lane-map/SKILL.md` is deliberately thin: cites `method.config.md § Lane map URL` (new key — one map can serve N repos sharing a Linear board, so the URL lives in config, never as a body constant) for the value, points to the SOP for conventions, and ships fill-in-the-blank sections for the curation that can't be derived from the board. **Verified against a real sync, not just reviewed:** a scratch consumer plus a `$TEMP` built from the actual (uncommitted) working tree via `SYNC_METHOD_TEMP` — bypassing `git clone`, which would only see committed history — confirmed a first sync scaffolds and registers the skill, a second sync leaves a locally-edited copy byte-for-byte untouched even after the upstream template changes in between, no false "possibly removed" warning fires on a clean run, and `--dry-run` reports intent without writing. **Two sub-decisions resolved against existing code precedent rather than guessed:** the mechanism is a generic manifest-driven mode, not a `lane-map`-hardcoded one-off — investigating `sync-method.sh` first found the generic and one-off implementations cost the same few lines, so genericity was free; and the new `Lane map URL` config key is a plain row in the existing `## V2 keys` table (matching `Financial review WIT` / `Migration dir`), not a commented-out block like `Skip rules` — that pattern is reserved for keys that change sync *behavior*, and this one only changes what a skill reads at runtime. `sop/Skills_SOP.md` documents the mechanism generally (`§ Scaffold-once skills`) and lists `/lane-map` in the portable-skills table. New smoke check validates every `.scaffold-once-skills` entry has a matching `skills/<name>/SKILL.md`. Smoke 199 → **200**.

## v4.28.1 — 2026-08-02

> **A skill may cite `method.config.md` for a *value*; it must never cite it for *prose*.** `method.config.md` is project-owned and `sync-method.sh` deliberately never touches it — so a conceptual section added to `method.config.template.md` reaches *new* projects (which copy the template) and **no existing consumer, ever**. v4.28.0 shipped exactly that defect: `/roadmap-create` told the reader "Both shapes are documented in `method.config.md § Initiative Surface → Board shapes`", a subsection that existed only in the template and was therefore **dangling on both SiteLine and Piper the day it shipped**. The failure mode is silent — a session follows the pointer, finds nothing, and proceeds on whatever it already believed. Caught in review of the SiteLine sync PR (#794). The fix is structural rather than a one-line repoint: the explanation moved to **`sop/Linear_SOP.md § Board shapes`** — a synced file, present in every consumer — which now also carries the layer's order-sensitive retirement path, and the two skill pointers (`/roadmap-create`, `/roadmap-review`) target the SOP while citing config only for the *value* that says which shape this project is in. The template keeps a short summary for someone filling in config, now explicitly marked as non-canonical so the two copies can't drift into contradiction. **The rule is new and general** — `sop/Skills_SOP.md` § "Cite `method.config.md` for values, never for prose", with the tell spelled out: "read § X for your team ID / lane size" is a value lookup and correct; "§ X documents how this works" is documentation and wrong. **Guarded**: a new smoke check parses every `` `method.config.md § …` `` pointer in every skill and fails unless the leaf resolves to a template section carrying a `| **Key** |` row, or to such a key itself. It found one further pre-existing pointer of a different shape (`§ Migration dir`) which is legitimate — that names a config key directly rather than a section — so the guard accepts both forms. Negative-tested by reintroducing the v4.28.0 pointer and confirming the suite goes red; the *first* negative test was vacuous (a `str.replace` whose target didn't match, silently doing nothing and reporting green), which is the same see-the-failure discipline `pipekit-tooling.md` documents and is worth noting as its own near-miss. Docs, skills, and tests only — no `bin/pk` behavior change. Smoke 198 → **199**.

## v4.28.0 — 2026-08-02

> **A Linear project that keeps accepting work is invisible to the roadmap walk, and nothing in Pipekit said so.** The `i{N}.`→`P{N}.` walk reads *into* a project but cannot see *inside* one: it names the lowest live project and stops. That is fine when a project is a completable batch and catastrophic when it's a standing pool. Anchor: SiteLine, 2026-08-02 — **98 of 162 open issues (60%) sat in three pool projects** (`Security & Hardening` alone held 62), so `pk status` pointed at `i2 → I2.P3 Export Improvements`, idle since 07-17, while 100% of live work was somewhere it structurally could not report. `In Progress` and `In Dev` both read 0. The board stopped being the execution authority and a gitignored `Temp/Notes.md` grew into the real one — lanes, function-level serialization tables, head-of-lane tracking — which is precisely the state a tracker exists to prevent. **The naming contract did not change and `bin/pk` needed no code change** (verified live: `pk status` derives `i3. Access Integrity → I3.P1. XSS output-encoding` off the reorganized board with zero edits, and behaves *better* against completable projects). What changed is what the levels *mean*: an initiative is an ordered, **completable release phase** (phases now map 1:1 to initiatives, so the old `ROADMAP.md` "Phase A/B/C" altitude moved onto the board); a project is a **lane** — a completable batch of 3–8 issues *cut* from the backlog, with its serialization notes in `content`; an unprefixed initiative is a long-running theme allowed never to finish; and **"no project" becomes the correct resting state for uncut work**, classified by a new `Area:` label group. The generalizable rule, now stated in `method.config.template.md`, `sop/Linear_SOP.md`, `/roadmap-create`, and `/phase-plan`: **completability lives at the project level; eternity lives at the theme/label level.** A project is created only for work already cut into a lane, never as a bucket to file things into. **`/linear-hygiene` gets the sharpest change, because its old homing rule pushed the wrong way** — it batch-homed orphans *into projects*, which is how pools grow. On the lanes model an orphan's home is a lane only if it fits that lane's completable scope ("same domain" explicitly does not count; the test is whether the lane can still be called done with it in it), otherwise **no project plus the right `Area:` label**, and the skill may never create a project. Its `Roadmap: *` guardrail — which protected labels a lanes-model board doesn't have — is replaced by the `Area:` successor, and the successor is **bidirectional** where the original was not: never rewrite an Area label because a lane changed, and never infer one *from* a lane, since a lane may legitimately mix areas. Four new detect-only checks (pool smell, spent lane, walk-skip hazard, status drift) report to a human and are never auto-applied — cutting or completing a lane is a planning act with roadmap consequences, so it belongs to the new **`/phase-plan --cut`**: survey the Area backlog, propose a coherent 3–8 issue slice, name it `I{N}.P{M}.`, write serialization notes, stamp run order, sanity-gate the counts, then create. `/phase-plan` also gains the initiative-close tail pattern (spec-approved leftovers → a named close-out lane in the next initiative; unspecced → back to the Area backlog) and the number-reuse rule for rolling the pointer forward. **Two things we deliberately did not do.** `pk` did **not** learn cycle-awareness: active-cycle membership is *derived* from run-order stamps, so teaching `pk` to read cycles would rebuild the second ordering mirror this whole model exists to remove — the same disease as running the phase-label layer alongside it, which is now explicitly forbidden rather than merely unnecessary. And placeholder projects are detected by a **description convention** (the literal word `placeholder`) rather than a config list, for the same reason: a list is one more mirror that drifts. Those empty projects are load-bearing, not clutter — an initiative whose projects are all completed reads as done to the walk and gets **skipped entirely**, which is the highest-consequence defect in the model and now its own `/roadmap-review` check. **One correction to the handover's own design:** it specified that a lane's active-cycle member must equal the lane's first non-done `sortOrder` stamp, while separately documenting that *a workflow-state change re-ranks `sortOrder`*. Since `pk branch` and `pk ship` are state changes, ordinary pipeline motion destroys the stamp and that check would false-positive after every normal transition. Resolved by scoping the invariant: **the stamp is authoritative for queued issues only** — once an issue is in flight its position is its state, not its stamp. Also fixed a latent config-shape bug found while writing the layer's retirement path: **`Roadmap source` lived inside the commented-out `### Phase Label Layer` block, but `/roadmap-review` Phase 2.5 reads it and is explicitly *not* gated on that section** — so the documented retirement ("comment the table, keep `Roadmap source` active") was impossible to execute, and retiring the layer would have silently repointed checkbox reconciliation at the default path. The key now lives in its own uncommented subsection. Ten Linear API gotchas from the live migration are recorded in `sop/Linear_SOP.md` § API gotchas, sorted by failure mode rather than by tool — the silent-data-loss class first (255-char `description` cap, state-changes re-ranking `sortOrder`, cycle-add moving an issue out of Backlog, Triage-state issues invisible in project views and excluded from scope counts, view preferences accepting unknown keys without error), then MCP gaps needing a GraphQL fallback (`archiveInitiative`, label delete), then Linear's 10k query-complexity cap and the sanity-gate-before-batch-mutation pattern. Docs and skills only — no `bin/pk` behavior change. Smoke 198, unchanged.

## v4.27.2 — 2026-08-02

> **`/verify` Step 7 rendered `reality-check.md` as prose, so its `sha:` line went missing ~14% of the time — and that line is what the v4.27.1 drift gate falls back to.** `verify-complete.md` is written only on PASS and is occasionally never committed (SiteLine PIPER-490), so `pk_verify_sentinel_latest` falls back to `reality-check.md`; when its `sha:` is absent the gate silently degrades to "no evidence found" and waves the branch through. The failure rate is measurable and was **rising**: across SiteLine's committed artifacts on `main` (2026-08-02), the Step 8 **heredoc**-written `verify-complete.md` carried a `sha:` line in **191 of 192** files, while the prose-rendered `reality-check.md` managed only **167 of 195** — 8% missing in June, 24% in July, 33% in August. The template was never wrong; it always said `- sha: <git rev-parse HEAD>`. Nothing in a prose template *fails* when a line goes missing, which is the whole point: the two files sit in the same skill, are written seconds apart, and differ only in that one is generated by a heredoc and the other by a model rendering markdown. Step 7 now writes the header block via a heredoc interpolating `$SHA=$(git rev-parse HEAD)`, exactly mirroring Step 8, and only the prose sections below it stay model-rendered (nothing parses those). Also fixed a latent instance of the same class: **`$STATUS` was consumed by Step 8's sentinel-write condition but assigned nowhere in the skill** — an unset `STATUS` silently skips the sentinel entirely, which is one plausible route to PIPER-490's missing file; Step 7 now assigns it explicitly, with the status-logic rule inline. Four smoke guards pin the heredoc, the `$SHA` interpolation, the `git rev-parse` assignment, and the `STATUS` assignment, so Step 7 cannot drift back to prose. No `bin/pk` behavior change. Smoke 194 → **198**.

## v4.27.1 — 2026-08-02

> **v4.27.0's gate was looking in the wrong place, and `pk done`'s half of it could never fire.** Two defects, found within hours of shipping — one by SiteLine's automated review of the sync PR, one by investigating why SiteLine PIPER-490 had no sentinel. **(1) The evidence lookup globbed the working tree only.** `pk ready` is documented as runnable from the parent repo, and `pk done` runs from the parent **by design** (it deletes the worktree) — and the parent's checkout does not contain the feature branch's `Logs/Verify/` artifacts at all. So `pk done`'s post-merge drift report was blind on every single invocation, and `pk ready` was blind whenever run the documented way. Reproduced on SiteLine: `feature/PIPER-553-scope-client-retrieval` carries `Logs/Verify/20260802/PIPER-553/verify-complete.md`; `main`'s working tree has none, and the old lookup returned nothing. `pk_verify_sentinel_latest` now takes an optional ref and searches the working tree **and** that branch's tree, with preference applied **per artifact rather than per source** — an authoritative `verify-complete.md` on the branch outranks a weaker `evidence.txt` that merely happens to be in the checkout. Both sources are load-bearing: a git-only read would have missed PIPER-490, whose sentinel was written to disk, honestly consumed by `pk ship`, never staged, and then destroyed with the worktree by `pk done`. **(2) `cmd_done` restated the blocking class list inline** as `source|migrations`, so it never matched `unreachable` after v4.27.0 taught `cmd_ready` to block on it — a branch rebased into an orphaned sentinel could merge with the post-merge audit comment silently skipped. `pk_verify_drift_blocks` was extracted in v4.27.0 precisely to be the single source of truth and then used at only one of its two call sites; `cmd_done` now calls it, and a smoke guard fails the build if either site restates the list (negative-tested: reintroducing the inline `case` trips it). Note defect 2 was masked by defect 1 — the report never fired from the parent regardless of class, so fixing either alone would have left the audit trail broken. **Not fixed here, deliberately:** `/verify` writes `verify-complete.md` but nothing instructs committing it, which is the root cause of PIPER-490's vanished evidence. That is a skill-behavior change with its own blast radius and belongs in its own release, not smuggled into a patch. Smoke 186 → **194**.

## v4.27.0 — 2026-08-02

> **The ship→merge window was unguarded: `pk ship` proves the verify sentinel matched at ship time, and nothing re-checks it while the PR is open.** Commits that land during review — review fixes, UAT fixes — move HEAD past the sentinel, so a PR can merge carrying a PASS artifact that describes code no longer in the diff. Anchor: SiteLine PIPER-490 (PR #774, 2026-07-31). `/verify` PASS at `eca992bb`; `pk ship` gated correctly at 21:55Z; then `4f47b160` rewrote **72 lines of the RLS migration's authorization predicate** at 22:28Z; merged 05:41Z. The PR review that read the evidence quoted its sha verbatim and never noticed HEAD had moved — a reviewer trusting the artifact is exactly who this fools. Three new helpers in `bin/pk`: **`pk_verify_drift_class`** (pure, stdin + args only, same house pattern as `pk_gate_verdict`) classifies the post-verify diff as `fresh` / `docs` / `source` / `migrations`, taking the migration dir from `method.config.md` so it stays portable across migration tools; **`pk_verify_sentinel_latest`** finds the newest verify evidence for an issue; **`pk_verify_drift_report`** resolves and compares. `pk ready` now **refuses** on `source`, `migrations`, or `unreachable` drift (`--force` flips anyway and logs a Linear audit comment, matching the ship gate's convention; `PK_VERIFY_BYPASS=1` also honored), and `pk done` reports drift to Linear post-merge — non-blocking there by design, since the merge already happened and the point is the record. Migrations get their own callout: a migration edited after the verify that exercised it is not merely stale evidence, it violates the frozen-file invariant, whose freeze begins at first application to **any** database including a local one. **The sentinel lookup deliberately falls back past `verify-complete.md` to `evidence.txt` / `reality-check.md`** — PIPER-490 had no `verify-complete.md` at all (it shipped past that gate via `--force`), and its verified sha survived only as `# sha:` and `- sha:` in those two files; keying solely on the sentinel would have made this gate structurally blind to its own anchor case. Short shas are resolved before comparison, so a `reality-check.md`-sourced sha can't false-block a clean branch. **`unreachable` fails closed, and the block/allow contract is its own pure helper (`pk_verify_drift_blocks`) so that stays testable** — the first cut let a rebased branch through with only a stderr warning, which SiteLine's review of the sync PR (#790) caught. The deciding argument is consistency rather than general fail-closed instinct: `pk ship`'s gate (`pk_verify_sentinel_for_head`) demands an *exact* sha match, so a rebased branch cannot ship at all; letting it flip to Ready made this gate strictly more permissive than the one it exists to reinforce, and a rebase immediately before requesting review defeated the feature outright. `none` (no evidence at all) stays non-blocking — `pk ship` already gates that — but now prints instead of staying silent, since silence is how a forced ship goes invisible downstream. Verified by replaying both real PRs: #774 classifies `migrations` (blocks, naming the rewritten migration), #786 classifies `docs` (benign — only verify artifacts moved, so the gate does not fire on the ordinary verify-docs commit). Also this release: `pipekit-tooling.md` § "The Verification Method Must Be Able to See the Failure" gains its converse — **can this check *cheat*?** A `git worktree` shares the parent repo's object database and refs, so a worktree detached at an old commit still reads every later commit; a blind cross-model review trial run this session read the follow-up fix PRs and cited them at confidence 100 before the leak was caught. **Provenance, and the non-adoption:** all of this came out of a pre-registered trial of Codex `gpt-5.6-sol` as a second-AI antagonistic reviewer. It **missed** the defect it was tested on and reported confidence 100 on residuals the PR itself already documented — so it was **not adopted** anywhere in the pipeline. What it did reliably surface, on both PRs, was stale *process artifacts*, because a model outside Pipekit treats verify evidence as a claim to check rather than as trusted context. That asymmetry is real; the fix for it is this deterministic gate, not a vendor. Smoke 161 → **186**.

## v4.26.4 — 2026-08-01

> **`/02-light-spec-revise`'s documented agent-reply format caught up with reality.** The skill's Phase 2 documented two verdict formats — `### Verdict` headings (Format A) and bold inline bullets (Format B) — while `pk spec-cycle`'s trigger explicitly demands, and the agent actually returns, a bare first-line `Verdict: Pass|Revise` followed by plain `Field: value` lines. Worse, Phase 1 *identified* agent comments by `^###\s+Verdict`, which matches nothing the agent now sends — author-name matching was the only working signal. The reply also arrives as a **threaded child comment** under the trigger, so a `comments(last:1)`-style read returns the trigger instead of the verdict (`bin/pk` fetches the full comment list and filters by author + since-timestamp, so it was always immune — the drift was skill-doc-only). Documented the plain-line shape as primary (Format C), demoted A/B to legacy (A stays live on nudge-path re-reviews, where no trigger restates the format and the agent falls back to its `templates/spec_review_skill.md` § Output Format rubric — deliberately left unchanged, since reconciling it means re-installing the Linear agent config in every consumer), extended every field locator across all three shapes, fixed the identification regex, and documented the threading gotcha in Phase 1. Surfaced on PIPER-517's Revise review (SiteLine, 2026-08-01). Doc-only; no `bin/pk` behavior change. Smoke 161, unchanged.
...
> **Four findings from a competitive scan and a log sweep, all pipekit-side, none new capability.** (1) **`/verify` Step 5's sensitive-content grep matched the whole diff, not just what changed.** `SECURITY DEFINER`/`GRANT EXECUTE`/`CREATE POLICY`/etc. tripped the antagonistic-review auto-trigger even when the term only appeared in an unchanged context line or a removed one — a file that merely *listed* one of these terms, never touched it, still fired review. Now scoped to added lines only (`grep '^+'` before the keyword match). Flagged by SiteLine's PIPER-494 and never actioned. (2) **`pk_linear_tier` folded "no `tier:` label" and "Linear unreachable" into the same silent `standard` default** — indistinguishable from an issue actually labeled `tier:standard`. The function now returns empty on either miss; `/verify` Step 0 owns the fallback and prints a visible warning when it's guessing the tier, so an issue created outside `/light-spec` (which is what assigns the label) can't silently ride through at whatever tier happens to be the default. Two new smoke cases cover the no-silent-default contract. (3) **Documented a real Supabase CLI gotcha, verified against the installed CLI (v2.110.0):** the `--agent` flag defaults to `auto`, and a script that pipes `supabase migration list` into a JSON parser without pinning it can work when an AI session runs it and false-fail for a human in a terminal, or vice versa. Anchored to SiteLine PIPER-533 (2026-07-31), a real incident in `scripts/local-dev/reset-db.sh`. New bullet in `pipekit-tooling.md` § "The Verification Method Must Be Able to See the Failure." (4) **Closed PIPER-494's other flagged gap — "two definitions of sensitive diverge" — as documentation, not code.** `/verify` Step 5's auto-trigger classifier, `/security-gate`'s six categories, and a project's own CI sensitivity check (e.g. SiteLine's doc-requirement tiers) are three deliberately separate mechanisms gating three different things; they were never meant to converge on one taxonomy, and now say so in `/verify` Step 5's docs. Unifying them into the `[verify.sensitive_paths]` override block stays deferred (v2.7.1+) — a materially bigger task than this release. Also this release: closed the TDD Red Flag's "or at minimum concurrently" hedge (`pipekit-discipline.md`) — surfaced by a periodic scan of the AI-SDLC framework landscape (obra/superpowers, GitHub Spec Kit, BMAD-METHOD, GSD) that otherwise found nothing worth adopting; the standing enforcement-over-persistence thesis (v0.5, 2026-05-29) held. Smoke 158 → **161**.
...
>
> **Also in this release — `/verify`'s pre-deploy gate could not report a failure.** Found in a sweep of SiteLine's session logs, independently recorded twice (PIPER-425, 2026-07-29). Step 3's `run_gate_command` read its exit code from `${PIPESTATUS[0]}`, but **zsh has no `PIPESTATUS`** — it spells the array `pipestatus` and indexes from 1 — and Claude Code's Bash tool runs zsh on macOS. So the expansion was empty: `printf 'exit: %d'` wrote `exit: 0` for every command, `return $rc` degenerated to `return` and handed back the *printf's* status, and `GATE_RC` never left 0. A red gate was byte-indistinguishable from a green one, `evidence.txt` preserved the false `exit: 0` as the durable record, and since `/verify` PASS writes the `verify-complete.md` sentinel that `pk ship` hard-gates on, a failing test suite could ship. Reproduced verbatim before fixing (`exit 7` → `GATE_RC=0` under zsh, `GATE_RC=7` under bash) and the fix re-tested in both shells. Fix is one line: read `$?`, which is correct in both shells because Step 3 already sets `set -o pipefail` immediately above. The `tier:quick` branch was never affected — it runs the gate unpiped and reads `$?` already.
>
> **Two more from the same sweep.** (1) **`pk ready` now says when a reviewer fires *once*.** v4.25.0 made the command honest about *which* reviewers run; it was still misleading about *when*. A workflow whose `pull_request` types carry `ready_for_review` but not `synchronize` reviews the Draft→Ready flip and nothing pushed afterwards — so the commits that fix the reviewer's own findings are precisely the ones nothing reviews, and the PR shows no sign of it. Recorded independently in three SiteLine session logs; PIPER-345's `879748e5` and PIPER-499's final head `778edbec` both merged unreviewed this way. New `pk_review_workflows_without_sync` probes for it and `pk ready` names any such workflow plus the recovery (`gh pr ready --undo <#> && gh pr ready <#>` re-fires the event on the current head). Reporting, not closing: adding `synchronize` trades CI minutes for reviewing every push, which is a project-level call. Uses `if grep` rather than `grep && continue` — under `set -euo pipefail` a bare `cmd && continue` whose left side fails returns non-zero as a complete command and would abort `pk` *after* the Draft flip already landed, the same latent class the v4.25.0 probe shipped with and its tests caught. (2) **`/verify` Step 3.5 — a green assertion is not coverage until something made it red.** A passing gate proves the suite ran, not that any of it can fail. Any test *cited* as evidence (Linear comment, PR body, `evidence.txt`, security-gate report) must be mutated and shown to redden on its named assertion rather than on a crash — a claim-time obligation, not a whole-suite audit. Three shapes named, each observed shipping: the tautology, the fixture that cannot violate, and the crash that reads as a failure. Anchor: PIPER-491 cited two unfailable assertions as security evidence and then repeated the same vacuous-fixture mistake in the pass writing the lesson up; PIPER-497 shipped four. Placed in `/verify` (demand-loaded, at the moment the green-suite claim is made) rather than the always-on rules, on the same reasoning as the drift signals. (3) **`pk ready` warns when the PR conflicts with its base.** GitHub cannot build `refs/pull/N/merge` for a conflicting PR, so it dispatches **zero** `pull_request` workflows — no failure, no message, and the check board keeps whatever it last showed. SiteLine PIPER-174 merged with `tests-required`, its *only* required gate, never having run and nothing on the PR saying so; the board read settled because a `pull_request_target` workflow was the sole reporter. The check runs **before** the already-Ready early return, because that PR was already Ready when it conflicted — placed after, it would have missed the motivating case exactly. Warns, never blocks: a conflicting PR is a legitimate intermediate state and resolving it is the user's call. Silent on `MERGEABLE` *and* on `UNKNOWN` — GitHub computes mergeability asynchronously, and asserting a state pk did not observe is the precise failure this surface exists to prevent; silence is not a claim, a false "looks fine" would be. Worth noting for anyone assuming a hosting change dissolves this: it is a GitHub Actions merge-ref mechanic, independent of where the app deploys, and a preview build that triggers off the branch head rather than the merge ref will keep posting a green check on a PR whose suite never ran. Smoke 145 → **157**. One of these fixes was itself caught by CI rather than locally: the conflict warning first read its integration branch with `pk config …`, shelling out to whatever `pk` is on PATH instead of using the in-script `pk_integration_branch`. It passed on the author's machine *only because* the installed `pk` is a symlink into another repo's working tree, and failed on a clean runner where no `pk` exists — the substitution dying under `set -euo pipefail` and suppressing the whole warning. Line 1286 was the sole self-call in `bin/pk`; a structural smoke test now asserts there are none, since the behavioural tests can only catch it on a machine without `pk` installed.
