...
**v2.7.0** — Last updated: 2026-06-05 08:45  *(v2.7.0 final — stable tag of the rc1–rc6 arc. Enforcement-substrate hardening: `/verify`'s migration flag and `/pr-fix` now carry a verdict instead of punting a raw `git show` to the human. New `/pk-express` — idea→Draft-PR autopilot for **simple** WITs (tier-guarded; quick/standard only; five attention gates), validated live on SiteLine POC-14. `/pr-fix` is pluggable-engine (pr-review-toolkit default, builtin fallback) with two-axis severity×confidence triage + git-history/prior-PR-comment historical finders. `pk branch` symlinks nested per-app env files; `pk doctor` false-ship cross-check; `/light-spec` publishes to the configured `Spec ready state`. No code change vs rc6 — the builtin `/pr-fix` fallback was re-validated as the last gate before cutting)*
