...
**v2.7.0-rc5** — Last updated: 2026-06-03 13:30  *(log-mining release from Piper + SiteLine: `pk doctor` now flags **false-ships** via a Linear↔git built-state cross-check (Done/UAT WITs with no real commits — the mirror of `/strategy-sync` 4b); `pk done` finish completed — parent-branch reset, running-stack advisory, softened migration check, and an optional `Deploy command` reminder for script-deploy projects ("merged ≠ deployed"); four discipline lines — migration pre-merge re-check, cmux stale-ref ground-truth fallback, vendor-UI-state red flag, MCP-in-`.mcp.json` worktree note. Carries forward rc4: `/verify` migration-flag self-review + `pk branch` nested env symlinks; rc3: `/pr-fix` historical finders + `pr-review-toolkit` managed dependency; rc1: discipline substrate, evidence-gated `/verify`)*
