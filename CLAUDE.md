...
VBW is fully retired: the executor went in v4.0.0 (native-on-Workflow is the sole executor), the plugin was decoupled in v4.2.0, and the phase surface became Linear-native in v4.1.0. Pipekit owns the whole surface:

- **Pipekit owns** Linear issues, the **phase surface** (Linear Initiatives `i{N}.` → Projects `I{N}.P{N}.` → Issues; v4.1.0, project `I{N}.` prefix added v4.5.0), strategy docs, and `method.config.md`. "What's next?" is read live from Linear via `pk next` (derives the current phase from the initiative/project hierarchy); v2 retired the `NEXT.md` mirror, v4.1.0 retired `PHASES.md`/`linear-map.json`.
...
Full phase model in `method.md` (§ Phase Surface Ownership).
...
| Command | Purpose |
|---------|---------|
| `pk next` | Phase-aware: derives the current phase live from the Linear-native surface (`i{N}.` initiative → `I{N}.P{N}.` project, by numeric name prefix; bare `P{N}.` and legacy `PHASES.md`/`linear-map.json` fall back), queries Linear, groups by status (In Progress / Approved / Needs Spec) with per-group hints. Replaces v1's `cat NEXT.md`. |
