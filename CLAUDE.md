...
**v2.7.0-rc6** — Last updated: 2026-06-03 16:45  *(new `/pk-express` — idea→Draft-PR autopilot for **simple** WITs: chains `/brainstorm` → `/light-spec` (auto-cycle to Approved) → `pk branch` → `/work` (auto verify+ship), advancing on success and stopping only at five attention gates (not-Now, **tier:heavy refusal**, spec stalemate, verify flags, Draft PR); quick/standard tier only; validated end-to-end on a live WIT. Plus a load-bearing `/light-spec` fix — publishes to the configured `Spec ready state` not a hardcoded `Specced`, so two-state boards work; and a `pipekit-cmux` lesson — watch worker turn-end, not narration keywords. Carries forward rc5: `pk doctor` false-ship cross-check + completed `pk done` finish; rc4: `/verify` migration self-review + `pk branch` nested env symlinks; rc3: `/pr-fix` historical finders)*
