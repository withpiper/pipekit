...
**v3.1.0** — Last updated: 2026-06-10  *(**v3.1.0 — distribution-layer hardening:** `bin/pk` smoke suite + CI gate and a global `--help` guard (`pk ship --help` once actually shipped); `pipekit/.local-skills` manifest so the sync distinguishes local-by-design skills from removed upstream ones; `pk doctor` upstream-staleness warning; the hand-curated visual roadmap (`NEXT.md`/`ROADMAP.md`) re-legitimized as a first-class optional human-owned artifact — `pk next` stays the operational truth. **Carries Pipekit 3.0 — native-on-Workflow is the default executor; VBW is an optional backend.** `/work` defaults `Backend:` to `native` when unset; planning is `/work`'s inline step regardless of backend, and `Backend: vbw` dispatches only `vbw-dev` (never `vbw-lead`/`vbw-qa`). Justified by the round-2 head-to-head (POC-48, tier:heavy financial parity): native matched-or-beat VBW-the-full-system on first-pass correctness at ~1/5 wall-clock and a fraction of the tokens — see `experiments/poc-48-roundtwo/`; reinforced by the POC-57 split verdict (`experiments/poc-57-backend-ab/`). The deep-analysis safety net is the gate layer (`/financial-review`, `/pr-security-review`), which both backends run. Consolidates the rc cycle: rc2's `/pk-bug` branch-early + backend-agnostic Linear review layer (`templates/linear_guidance.md` + `templates/spec_review_skill.md`, v5.2); rc3's ownership-model `vbw-lead` scrub; final completes that scrub in `GUIDE.md` + `agents/plan-reviewer.md`. Carries v2.8.0 (rc1–rc3): `/financial-review` + checks-file split & config keys; `/security-review` finding-stage coverage + adversarial verification pass; CI reviewer-trigger hardening (`synchronize` self-heal + workflow-validation 401 warning); `/pk-bug` checkout guard; and v2.7.x: enforcement-substrate hardening, `/pk-express`, `/pr-fix` pluggable engine, merge-driven Linear transition)*
